...
export default function SelectDevice() {
  ...
  const startDeviceScan = useCallback(async () => {
    ...
    try {
      ...
      if (!systemSetting.bluetoothEnabled) {
        showToast({ tip: '蓝牙未打开，请先打开蓝牙', duration: 2000 })
        return
      }
      ...
      // 过滤设备名称并去重
      // 如果是ios设备 根据LocalName或name过滤，有一项符合条件即可
      if (isIOS) {
      ...
      showToast({ tip: '获取设备列表失败', duration: 2000 })
      setDeviceList([])
    } finally {
  ...
  return (
    <View className={styles.container} style={{ display: isActive ? 'block' : 'none' }}>
      <View className={styles.stepTitle}>当前步骤：扫描蓝牙设备</View>
      <DeviceList
