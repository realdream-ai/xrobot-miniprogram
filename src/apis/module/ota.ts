...
export default {
  ...
  // 获取单个OTA固件信息
  getOtaInfo(id: string, callback: (res: any) => void) {
    RequestService.sendRequest()
  ...
  // 保存OTA固件信息
  saveOta(entity: any, callback: (res: any) => void) {
    RequestService.sendRequest()
      .url(`${getServiceUrl()}/otaMag`)
      .method('POST')
      .data(entity)
  ...
  // 删除OTA固件
  deleteOta(id: string, callback: (res: any) => void) {
    RequestService.sendRequest()
      .url(`${getServiceUrl()}/otaMag/${id}`)
      .method('DELETE')
      .success((res: any) => {
  ...
  uploadFirmware(
    file: File,
    callback: (res: ApiResponse<{ url: string }>) => void
  ) {
  ...
  // 获取固件下载链接
  getDownloadUrl(id: string, callback: (res: any) => void) {
    RequestService.sendRequest()
